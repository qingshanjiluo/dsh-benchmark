...
assert.equal(mod.name, 'dsh-benchmark', 'plugin name export')
...
const registered = []
mod.apply({ tools: { register: def => registered.push(def) } }, { precision: 4, reportHeading: 'Benchmark Report' })
assert.deepEqual(registered.map(t => t.name).sort(), ['bench_compare', 'bench_report', 'bench_stats'], 'all three tools register')
